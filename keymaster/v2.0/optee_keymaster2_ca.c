...
TEEC_Result configure(uint32_t os_version,
    ...
    op.params[0].value.a = os_version;
    op.params[0].value.b = os_patchlevel;
    op.paramTypes = TEEC_PARAM_TYPES(TEEC_VALUE_INPUT, TEEC_NONE, TEEC_NONE,
            TEEC_NONE);
    res = TEEC_InvokeCommand(&KM_session, KM_CONFIGURE, &op,
            &ret_orig);
    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke KEYMASTER_CMD_CONFIGURE failed with res(%x), ret_orig(%x)\n", res, ret_orig );
    }
...
static TEEC_Result free_transient_object(TEE_ObjectHandle obj)
{
    ...
    op.paramTypes = TEEC_PARAM_TYPES(TEEC_VALUE_INPUT, TEEC_NONE, TEEC_NONE,
            TEEC_NONE);
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_FREE_TRANSIENT_OBJ, &op,
            &ret_orig);
    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_FREE_TRANSIENT_OBJ failed with res(%x), ret_orig(%x)\n", res, ret_orig );
    }
...
static TEE_Result allocate_transient_object(TEE_ObjectType obj_type, uint32_t obj_size, TEE_ObjectHandle* obj)
{
    ...
    res = TEEC_InvokeCommand(&KM_session,
            OPTEE_KEYMASTER_CMD_ID_ALLOCATE_TRANSIENT_OBJ,
            &op,
            &ret_orig);

    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_ALLOCATE_TRANSIENT_OBJ failed with res(%x), ret_orig(%x)\n", res, ret_orig );
    }
...
TEEC_Result populate_transient_object(TEE_ObjectHandle o, const TEE_Attribute *attrs, uint32_t attr_count)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_POPULATE_TRANSIENT_OBJ, &op,
            &ret_orig);
        ...
        uint32_t max_key_size)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_ALLOCATE_OPERATION, &op,
            &ret_orig);
    ...
    if (res == TEEC_SUCCESS) {
        *oph = (TEE_OperationHandle)(uintptr_t)op.params[0].value.a;
        ...
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_ALLOCATE_OPERATION failed with res(%x), ret_orig(%x)\n", res, ret_orig );
    }
...
TEEC_Result free_operation(TEE_OperationHandle oph)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_FREE_OPERATION, &op,
            &ret_orig);

    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_FREE_OPERATION failed with res(%x), ret_orig(%x)\n", res, ret_orig );
    }
...
TEE_Result set_operation_key(TEE_OperationHandle oph, TEE_ObjectHandle key)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_SET_OPERATION_KEY, &op,
            &ret_orig);

    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_SET_OPERATION_KEY failed with res(%x), ret_orig(%x)\n", res, ret_orig );
    }
        ...
        size_t *dst_len)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_SIGN_DIGEST, &op, &ret_orig);

    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_SIGN_DIGEST failed with res(%x), ret_orig(%x)\n", res, ret_orig );
    }
        ...
        size_t *dst_len)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_ASYMMETRIC_EN_DE_CRYPT, &op, &ret_orig);

    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_ASYMMETRIC_CRYPT failed with res(%x), ret_orig(%x)\n", res, ret_orig );
    }
        ...
        size_t signature_len)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_VERIFY_SIGNATURE,
            &op, &ret_orig);
    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_VERIFY_SIGNATURE failed with res(%x), ret_orig(%x)\n", res, ret_orig );
    }
        ...
        uint32_t paramCount)
{
    ...
    res = TEEC_InvokeCommand(&KM_session,
            OPTEE_KEYMASTER_CMD_ID_GENERATE_KEYPAIR,
            &op,
            &ret_orig);
    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_GENERATE_KEYPAIR failed with res(%x), ret_orig(%x)\n", res, ret_orig );
    }
        ...
        uint8_t** x509_data, size_t* x509_data_len)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_GET_KEYPAIR_PUBLIC,
            &op, &ret_orig);
    if (res != TEEC_SUCCESS) {
        LOG_D("%s:%d: Invoke OPTEE_KEYMASTER_CMD_ID_GET_KEYPAIR_PUBLIC failed with res(%x), ret_orig(%x)\n", __func__, __LINE__, res, ret_orig);
    }
    else {
...
TEEC_Result delete_key_ca(const uint8_t *id, uint32_t id_len)
{
    ...
    /* Delete Keypair */
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_DELETE_KEYPAIR, &op, &ret_orig);
    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_DELETE_KEYPAIR failed with res(%x), ret_orig(%x)\n", res, ret_orig);
        goto out;
    }
    ret = 0;
out:
        ...
        uint32_t obj_type, uint32_t key_len)
{
    ...
    /* Load Keypair */
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_LOAD_KEY, &op, &ret_orig);
    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_LOAD_KEY failed with res(%x), ret_orig(%x)\n", res, ret_orig);
        goto out;
    } else {
...
TEEC_Result query_key_existence_ca(const TEE_ObjectHandle handle)
{
    ...
    /* Delete Keypair */
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_QUERY_KEY_EXISTENCE, &op, &ret_orig);
    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_QUERY_KEY_EXISTENCE failed with res(%x), ret_orig(%x)\n", res, ret_orig);
        goto out;
    }
out:
    return res;
        ...
        size_t chunk_size)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_DIGEST_UPDATE, &op, &ret_orig);

    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_DIGEST_UPDATE failed with res(%x), ret_orig(%x)\n",
                res, ret_orig );
    }
        ...
        size_t *hash_len)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_DIGEST_DO_FINAL, &op,
            &ret_orig);

    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_DIGEST_DO_FINAL failed with res(%x), ret_orig(%x)\n", res, ret_orig );
    }
        ...
        uint8_t *id, uint32_t id_len)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_IMPORT_KEYPAIR, &op,
            &ret_orig);
    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_IMPORT_KEYPAIR failed with res(%x), ret_orig(%x)\n", res, ret_orig );
        goto out;
    }
        ...
        uint8_t *id, uint32_t id_len)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_IMPORT_SYMMETRIC_KEY, &op,
            &ret_orig);
    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_IMPORT_SYMMETRIC_KEY failed with res(%x), ret_orig(%x)\n", res, ret_orig );
        goto out;
    }
        ...
        TEE_OperationHandle* oph)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_MAC_KEYBLOB_INIT, &op_mac, &ret_orig);
    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_MAC_KEYBLOB_INIT failed with res(%x), ret_orig(%x)\n", res, ret_orig);
    } else {
        ...
        uint32_t key_len)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_MAC_INIT, &op_mac, &ret_orig);
    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_MAC_INIT failed with res(%x), ret_orig(%x)\n", res, ret_orig);
    }
        ...
        size_t chunk_size)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_MAC_UPDATE, &op, &ret_orig);

    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_MAC_UPDATE failed with res(%x), ret_orig(%x)\n",
                res, ret_orig );
    }
        ...
        size_t *hash_len)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_MAC_DO_FINAL, &op,
            &ret_orig);
        ...
        size_t hash_len)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_MAC_DO_FINAL_COMPARE, &op,
            &ret_orig);

    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_DIGEST_DO_FINAL_COMPARE failed with res(%x), ret_orig(%x)\n", res, ret_orig );
    }
        ...
        const void *iv, size_t iv_len)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_CIPHER_INIT, &op, &ret_orig);

    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_CIPHER_INIT failed with res(%x), ret_orig(%x)\n", res, ret_orig);
    }
        ...
        void *dst, size_t *dst_len, const uint32_t need_buffing)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_CIPHER_UPDATE, &op, &ret_orig);

    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_CIPHER_UPDATE failed with res(%x), ret_orig(%x)\n", res, ret_orig);
    }
        ...
        void *dst, size_t *dst_len)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_CIPHER_DO_FINAL, &op,
            &ret_orig);

    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_CIPHER_DO_FINAL failed with res(%x), ret_orig(%x)\n", res, ret_orig);
    }
        ...
        uint8_t** x509_data, size_t* x509_data_len)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_GET_KEYPAIR_PUBLIC,
            &op, &ret_orig);
    if (res != TEEC_SUCCESS) {
        LOG_D("%s:%d: Invoke OPTEE_KEYMASTER_CMD_ID_GET_KEYPAIR_PUBLIC failed with res(%x),\
                ret_orig(%x)\n", __func__, __LINE__, res, ret_orig);
    } else {
        blen = op.params[1].tmpref.size;
        *x509_data = (uint8_t*)malloc(blen);
        ...
        size_t payload_len)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_AE_INIT, &op, &ret_orig);

    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_AE_INIT failed with res(%x), ret_orig(%x)\n", res, ret_orig);
    }
    return res;
}
        ...
        const void *aad, size_t aad_len)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_AE_UPDATE_AAD, &op, &ret_orig);

    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_AE_UPDATE_AAD failed with res(%x), ret_orig(%x)\n", res, ret_orig);
    }
        ...
        const uint32_t need_buffering)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_AE_UPDATE, &op, &ret_orig);

    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_AE_UPDATE failed with res(%x), ret_orig(%x)\n", res, ret_orig);
    }
        ...
        size_t *tag_len)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_AE_ENCRYPT_FINAL, &op,
            &ret_orig);

    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_AE_ENCRYPT_FINAL failed with res(%x), ret_orig(%x)\n", res, ret_orig);
    }
        ...
        const void *tag, size_t tag_len)
{
    ...
    res = TEEC_InvokeCommand(&KM_session, OPTEE_KEYMASTER_CMD_ID_AE_DECRYPT_FINAL, &op,
            &ret_orig);

    if (res != TEEC_SUCCESS) {
        LOG_D("Invoke OPTEE_KEYMASTER_CMD_ID_AE_DECRYPT_FINAL failed with res(%x), ret_orig(%x)\n", res, ret_orig);
    }
