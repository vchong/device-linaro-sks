 ...
 */
static int optee_keymaster_open(const hw_module_t* module, const char* name, hw_device_t** device) {
    if (strcmp(name, KEYSTORE_KEYMASTER) != 0) {
        return -EINVAL;
    }

    OpteeKeymasterDevice* dev = new OpteeKeymasterDevice(module);
    if (dev == NULL) {
        return -ENOMEM;
    }
