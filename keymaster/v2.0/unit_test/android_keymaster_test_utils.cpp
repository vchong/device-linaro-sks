...
keymaster_error_t Keymaster2Test::FinishOperation(const AuthorizationSet& additional_params,
    ...
    keymaster_key_param_set_t out_params;
    keymaster_error_t error =
        device()->finish(device(), op_handle_, &additional_params, &inp, &sig, &out_params, &out_tmp);
    if (error != KM_ERROR_OK) {
...
class Sha256OnlyWrapper {
  public:
    Sha256OnlyWrapper(const keymaster2_device_t* wrapped_device) : wrapped_device_(wrapped_device) {
    ...
    keymaster2_device_t* keymaster_device() { return &device_; }
    ...
    static int close_device(hw_device_t* dev) {
        ...
        const keymaster2_device_t* wrapped_device = wrapper->wrapped_device_;
        delete wrapper;
        return wrapped_device->common.close(const_cast<hw_device_t*>(&wrapped_device->common));
    }

    static const keymaster2_device_t* unwrap(const keymaster2_device_t* dev) {
        return reinterpret_cast<const Sha256OnlyWrapper*>(dev)->wrapped_device_;
    }

    static keymaster_error_t add_rng_entropy(const struct keymaster2_device* dev,
    ...
    get_key_characteristics(const struct keymaster2_device* dev,
    ...
    static keymaster_error_t
    import_key(const keymaster2_device_t* dev, const keymaster_key_param_set_t* params,
    ...
    static keymaster_error_t export_key(const struct keymaster2_device* dev,  //
    ...
    static keymaster_error_t finish(const struct keymaster2_device* dev,  //
                                    keymaster_operation_handle_t operation_handle,
                                    const keymaster_key_param_set_t* in_params,
                                    ...
                                    keymaster_blob_t* output) {
        return unwrap(dev)
            ->finish(unwrap(dev), operation_handle, in_params, input, signature, out_params, output);
    }

    static keymaster_error_t abort(const struct keymaster2_device* dev,
                                   keymaster_operation_handle_t operation_handle) {
  ...
  private:
    ...
    const keymaster2_device_t* wrapped_device_;
    hw_module_t new_module;
    string new_module_name;
};

keymaster2_device_t* make_device_sha256_only(keymaster2_device_t* device) {
