...
/**
 * Test instance creator that builds a pure software keymaster2 implementations.
 */
 ...
 */
class LinaroKeymasterTestInstanceCreator : public Keymaster2TestInstanceCreator {
  public:
      keymaster2_device_t* CreateDevice() const override {
          const hw_module_t* mod;
          keymaster2_device_t* km2_device = NULL;
          ...
          if (rc) {
              ...
              rc = keymaster2_open(mod, &km2_device);
              if (rc) {
          ...
          if (km2_device) {
              ...
              error = device->SetHardwareDevice(km2_device);
              if (error != KM_ERROR_OK) {
                  std::cerr << "Error set keymaster2 device" << std::endl;
              }
          }
...
/**
 * Test instance creator that builds a SoftKeymasterDevice which wraps a fake hardware keymaster2
 * instance, with minimal digest support.
 */
class Sha256OnlyKeymaster2TestInstanceCreator : public Keymaster2TestInstanceCreator {
    keymaster2_device_t* CreateDevice() const {
        std::cerr << "Creating keymaster2-backed device that supports only SHA256";

        // fake_device doesn't leak because device (below) takes ownership of it.
        //keymaster2_device_t* fake_device = make_device_sha256_only(
        ...
        device->SetHardwareDevice(counting_keymaster0_device_);
...
#endif
    //InstanceCreatorPtr(new SoftKeymasterTestInstanceCreator)
    InstanceCreatorPtr(new LinaroKeymasterTestInstanceCreator)
    );

typedef Keymaster2Test CheckSupported;
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Values(
        ...
        InstanceCreatorPtr(new Keymaster0AdapterTestInstanceCreator(false /* support_ec */))));
#endif
TEST_P(Keymaster0AdapterTest, OldSoftwareKeymaster2RsaBlob) {
    // Load and use an old-style Keymaster2 software key blob.  These blobs contain OCB-encrypted
    // key data.
    string km2_sw = read_file("km2_sw_rsa_512.blob");
    EXPECT_EQ(486U, km2_sw.length());

    uint8_t* key_data = reinterpret_cast<uint8_t*>(malloc(km2_sw.length()));
    memcpy(key_data, km2_sw.data(), km2_sw.length());
    set_key_blob(key_data, km2_sw.length());
...
TEST_P(Keymaster0AdapterTest, UnversionedSoftwareKeymaster2RsaBlob) {
    // Load and use an old-style Keymaster2 software key blob, without the version byte.  These
    // blobs contain OCB-encrypted key data.
    string km2_sw = read_file("km2_sw_rsa_512_unversioned.blob");
    EXPECT_EQ(477U, km2_sw.length());
...
TEST_P(Keymaster0AdapterTest, OldSoftwareKeymaster2EcdsaBlob) {
    // Load and use an old-style Keymaster2 software key blob.  These blobs contain OCB-encrypted
    // key data.
    string km2_sw = read_file("km2_sw_ecdsa_256.blob");
    EXPECT_EQ(270U, km2_sw.length());
...
TEST(SoftKeymasterWrapperTest, CheckKeymaster2Device) {
    ...
    EXPECT_TRUE(good_fake_wrapper->Keymaster2DeviceIsGood());
    ...
    keymaster2_device_t* sha256_only_fake = make_device_sha256_only(
    ...
    EXPECT_FALSE(sha256_only_fake_wrapper->Keymaster2DeviceIsGood());
