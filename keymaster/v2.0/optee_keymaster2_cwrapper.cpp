...
#define TEE_OBJECT_TYPE_ALGO(x) x
...
keymaster_error_t KM2_secure_configure(
    ...
    result = configure(request.os_version, request.os_patchlevel);
    if (result != TEEC_SUCCESS) {
        LOG_D("%s: failed", __FUNCTION__);
        return KM_ERROR_SECURE_HW_COMMUNICATION_FAILED;
    }
 ...
 *	Input:  dev - 		pointer to keymaster2_device_t
 *		key - 		pointer to a keypair
 *		key_len - 	length of the keypair
 *	Output: optee_key - 	pointer to optee_key returned to HAL
 *	Returns: 0 - 		success
 *		-1 - 		failed
 */
int KM_secure_import_keypair(const keymaster2_device_t* dev __unused,
        const uint8_t* key, const size_t key_bin_len,
        optee_keyblob_t* optee_key)
{
    ...
    optee_key->key_len = EVP_PKEY_bits(pkey);
    switch (EVP_PKEY_type(pkey->type)) {
        case EVP_PKEY_DSA:
            optee_key->optee_obj_type = TEE_TYPE_DSA_KEYPAIR;
            break;
        case EVP_PKEY_RSA:
            optee_key->optee_obj_type = TEE_TYPE_RSA_KEYPAIR;
            break;
        case EVP_PKEY_EC:
            optee_key->optee_obj_type = TEE_TYPE_ECDSA_KEYPAIR;
            break;
        default:
    ...
    /* Generate Keypair */
    res = do_import_keypair_tee_ca(key, key_bin_len, optee_key->optee_obj_type,
            optee_key->key_len, optee_key->handle, sizeof(optee_key->handle));
    if (res != TEEC_SUCCESS) {
        LOG_D("do_import_keypair_tee_ca failed: %zd %x\n", key_bin_len, optee_key->optee_obj_type);
        goto out;
    }
    ret = 0;
out:
 ...
 *		-1 - 	failed
 */
int KM_secure_query_key_existence(const keymaster2_device_t* dev __unused, const TEE_ObjectHandle handle)
{
 ...
 *	Output: optee_key - 	optee_keyblob_t returned to HAL
 *	Returns: 0 - 		success
 *		-1 - 		failed
 */
keymaster_error_t KM2_secure_generate_key(
        const AuthorizationSet& key_description,
        optee_keyblob_t *optee_key,
        ...
        AuthorizationSet *sw_enforced)
{
    ...
    if (KM_ALGORITHM_RSA == algo) {
        ...
        if (TEEC_SUCCESS != generate_rsa_keypair_ca(optee_key->handle, sizeof(optee_key->handle), &rsa_params)) {
            LOG_D("%s:%d: generate_rsa_keypair failed: mod: %u, exp: %llu\n", __func__, __LINE__, rsa_params.modulus_size, rsa_params.public_exponent);
            ret = KM_ERROR_UNKNOWN_ERROR;
            goto out;
        }
        optee_key->key_len = rsa_params.modulus_size;
        optee_key->optee_obj_type = TEE_TYPE_RSA_KEYPAIR;
        ...
        if (TEEC_SUCCESS != generate_ec_keypair(optee_key->handle, sizeof(optee_key->handle), &ec_params)) {
            LOG_D("%s:%d: generate_ec_keypair failed\n", __func__, __LINE__);
            ret = KM_ERROR_UNKNOWN_ERROR;
            goto out;
        }
        optee_key->key_len = ec_params.field_size;
        optee_key->optee_obj_type = TEE_TYPE_ECDSA_KEYPAIR;
    } else if (KM_ALGORITHM_AES == algo) {
        if (TEEC_SUCCESS != generate_symmetric_key(optee_key->handle, sizeof(optee_key->handle),
                    optee_key->key_len, TEE_TYPE_AES)) {
            LOG_D("%s:%d: generate_symmetric_key failed\n", __func__, __LINE__);
            ret = KM_ERROR_UNKNOWN_ERROR;
            goto out;
        }
        optee_key->optee_obj_type = TEE_TYPE_AES;
        ...
        if (!dgst)
            ...
            optee_key->optee_obj_type = TEE_OBJECT_TYPE_ALGO(dgst);
        if (TEEC_SUCCESS != generate_symmetric_key(optee_key->handle, sizeof(optee_key->handle),
                    key_size_bits, optee_key->optee_obj_type)) {
            LOG_E("generate_symmetric_key failed: %d %x\n", key_size_bits, optee_key->optee_obj_type);
            goto out;
        }
        optee_key->key_len = key_size_bits;
    } else {
        LOG_D("%s:%d: Unsupport key type\n", __func__, __LINE__);
        goto out;
    }
    key_material.Reset(sizeof(optee_keyblob_t));
    memcpy(key_material.writable_data(), optee_key, sizeof(optee_keyblob_t));
...
const uint32_t rsa_algo_table_size = sizeof(rsa_algo_table) / sizeof(rsa_algo_table[0]);
...
const uint32_t aes_algo_table_size = sizeof(aes_algo_table) / sizeof(aes_algo_table[0]);
...
keymaster_error_t KM2_hmac_keyblob_init(
        keymaster_algorithm_t android_algo,
        keymaster_digest_t digest,
...
keymaster_error_t KM2_hmac_init(
...
keymaster_error_t KM2_hmac_update(
...
keymaster_error_t KM2_hmac_final(
        TEE_OperationHandle hmac_op,
        const void* chunk, size_t chunk_len,
        void* hash, size_t* hash_len)
{
...
keymaster_error_t KM2_hmac_final_compare(
        TEE_OperationHandle hmac_op,
        const void* chunk, size_t chunk_len,
        ...
        optee_keyblob_t* optee_key)
{
    ...
    optee_key->key_len = key_len * 8;
    res = do_import_symmetric_key_tee_ca(key, key_len, optee_key->optee_obj_type,
            optee_key->handle, sizeof(optee_key->handle));
    if (res != TEEC_SUCCESS) {
        LOG_D("do_import_symmetric_key_tee_ca: %x %x\n", res, optee_key->optee_obj_type);
        ret = KM_ERROR_UNKNOWN_ERROR;
        goto out;
    }
out:
        ...
        size_t payload_len)
{
    ...
    if (res != TEEC_SUCCESS) {
        LOG_E("KM2_ae_init: failed: %zd, %zd, %zd, %zd", nonce_len, tag_len, aad_len, payload_len);
...
keymaster_error_t KM2_ae_decrypt_final(
...
keymaster_error_t KM2_load_key(TEE_ObjectHandle *keyobj, const uint8_t *id, uint32_t id_len,
		uint32_t obj_type, uint32_t key_len)
{
    TEEC_Result res = TEEC_SUCCESS;
