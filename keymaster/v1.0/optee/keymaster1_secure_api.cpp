 ...
 *	Output: linaro_key - 	pointer to linaro_key returned to HAL
 *	Returns: 0 - 		success
 *		-1 - 		failed
 */
        ...
        linaro_keyblob_t* linaro_key)
{
    ...
    linaro_key->key_len = EVP_PKEY_bits(pkey);
    switch (EVP_PKEY_type(pkey->type)) {
        case EVP_PKEY_DSA:
            linaro_key->optee_obj_type = TEE_TYPE_DSA_KEYPAIR;
            break;
        case EVP_PKEY_RSA:
            linaro_key->optee_obj_type = TEE_TYPE_RSA_KEYPAIR;
            break;
        case EVP_PKEY_EC:
            linaro_key->optee_obj_type = TEE_TYPE_ECDSA_KEYPAIR;
            break;
        default:
    ...
    /* Generate Keypair */
    res = do_import_keypair_tee_ca(key, key_bin_len, linaro_key->optee_obj_type,
            linaro_key->key_len, linaro_key->handle, sizeof(linaro_key->handle));
    if (res != TEEC_SUCCESS) {
        LOG_D("do_import_keypair_tee_ca failed: %zd %x\n", key_bin_len, linaro_key->optee_obj_type);
        goto out;
    }
    ret = 0;
out:
 ...
 *	Output: linaro_key - 	linaro_keyblob_t returned to HAL
 *	Returns: 0 - 		success
 *		-1 - 		failed
 */
        ...
        AuthorizationSet *sw_enforced)
{
    ...
    if (KM_ALGORITHM_RSA == algo) {
        ...
        if (TEEC_SUCCESS != generate_rsa_keypair_ca(linaro_key->handle, sizeof(linaro_key->handle), &rsa_params)) {
            LOG_D("%s:%d: generate_rsa_keypair failed: mod: %u, exp: %llu\n", __func__, __LINE__, rsa_params.modulus_size, rsa_params.public_exponent);
            ret = KM_ERROR_UNKNOWN_ERROR;
            goto out;
        }
        linaro_key->key_len = rsa_params.modulus_size;
        linaro_key->optee_obj_type = TEE_TYPE_RSA_KEYPAIR;
        ...
        if (TEEC_SUCCESS != generate_ec_keypair(linaro_key->handle, sizeof(linaro_key->handle), &ec_params)) {
            LOG_D("%s:%d: generate_ec_keypair failed\n", __func__, __LINE__);
            ret = KM_ERROR_UNKNOWN_ERROR;
            goto out;
        }
        linaro_key->key_len = ec_params.field_size;
        linaro_key->optee_obj_type = TEE_TYPE_ECDSA_KEYPAIR;
    } else if (KM_ALGORITHM_AES == algo) {
        if (TEEC_SUCCESS != generate_symmetric_key(linaro_key->handle, sizeof(linaro_key->handle),
                    linaro_key->key_len, TEE_TYPE_AES)) {
            LOG_D("%s:%d: generate_symmetric_key failed\n", __func__, __LINE__);
            ret = KM_ERROR_UNKNOWN_ERROR;
            goto out;
        }
        linaro_key->optee_obj_type = TEE_TYPE_AES;
        ...
        if (!dgst)
            ...
            linaro_key->optee_obj_type = 0; //TEE_OBJECT_TYPE_ALGO(dgst);
        if (TEEC_SUCCESS != generate_symmetric_key(linaro_key->handle, sizeof(linaro_key->handle),
                    key_size_bits, linaro_key->optee_obj_type)) {
            LOG_E("generate_symmetric_key failed: %d %x\n", key_size_bits, linaro_key->optee_obj_type);
            goto out;
        }
        linaro_key->key_len = key_size_bits;
    } else {
        LOG_D("%s:%d: Unsupport key type\n", __func__, __LINE__);
        goto out;
    }
    key_material.Reset(sizeof(linaro_keyblob_t));
    memcpy(key_material.writable_data(), linaro_key, sizeof(linaro_keyblob_t));
    if (!key_material.key_material)
        return KM_ERROR_MEMORY_ALLOCATION_FAILED;
    ret = LinaroCreateKeyBlob(authorizations, KM_ORIGIN_GENERATED,
        ...
        TEE_OperationHandle* operation)
{
	...
	if (!dgst)
		...
		optee_obj_type = 0; //TEE_OBJECT_TYPE_ALGO(dgst);
        ...
        linaro_keyblob_t* linaro_key)
{
    ...
    linaro_key->key_len = key_len * 8;
    res = do_import_symmetric_key_tee_ca(key, key_len, linaro_key->optee_obj_type,
            linaro_key->handle, sizeof(linaro_key->handle));
    if (res != TEEC_SUCCESS) {
        LOG_D("do_import_symmetric_key_tee_ca: %x %x\n", res, linaro_key->optee_obj_type);
        ret = KM_ERROR_UNKNOWN_ERROR;
        goto out;
    }
out:
