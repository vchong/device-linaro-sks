...
//#include <linaro_keymaster1_keyblob_utils.h>
...
typedef UniquePtr<linaro_keyblob_t> Unique_linaro_keyblob_t;

__attribute__ ((visibility ("default")))
int linaro_terminate()
{
    //#ifdef USE_HW_KEYMASTER
#if 0
...
/* keymaster1 APis */
keymaster_error_t linaro_get_supported_algorithms(const struct keymaster1_device* dev __unused,
...
keymaster_error_t linaro_get_supported_block_modes(const struct keymaster1_device* dev __unused,
        keymaster_algorithm_t algorithm,
        keymaster_purpose_t purpose,
...
keymaster_error_t linaro_get_supported_padding_modes(const struct keymaster1_device* dev __unused,
        keymaster_algorithm_t algorithm,
        keymaster_purpose_t purpose,
        keymaster_padding_t** modes,
        size_t* modes_length)
...
keymaster_error_t linaro_get_supported_digests(
...
keymaster_error_t linaro_get_supported_import_formats(const struct keymaster1_device* dev __unused,
        keymaster_algorithm_t algorithm,
...
keymaster_error_t linaro_get_supported_export_formats(const struct keymaster1_device* dev __unused,
        keymaster_algorithm_t algorithm,
        keymaster_key_format_t** formats,
        size_t* formats_length)
{
...
keymaster_error_t linaro_add_rng_entropy(const struct keymaster1_device* dev __unused, const uint8_t* data __unused,
...
static keymaster_key_characteristics_t* LinaroBuildCharacteristics(const AuthorizationSet& hw_enforced,
...
keymaster_error_t linaro_generate_key(const struct keymaster1_device* dev,
        const keymaster_key_param_set_t* params,
        keymaster_key_blob_t* key_blob,
        keymaster_key_characteristics_t** characteristics)
{
    linaro_keyblob_t linaro_key;
    keymaster_error_t error = KM_ERROR_OK;
    ...
    memset(&linaro_key, 0, sizeof(linaro_keyblob_t));
    ...
    if (algorithm == KM_ALGORITHM_AES) {
        error = get_and_validate_aes_keygen_params(key_description, &linaro_key.key_len);
        CHK_ERR_AND_LEAVE(error, "get_and_validate_aes_keygen_params failed", out);
    }

    error = KM1_secure_generate_key(key_description, &linaro_key, &generated_blob, &hw_enforced, &sw_enforced);
    if (error != KM_ERROR_OK) {
    ...
    *key_blob = generated_blob.release();
    ...
    if (characteristics) {
        *characteristics = LinaroBuildCharacteristics(hw_enforced, sw_enforced);
        if (!*characteristics) {
...
out:
    if (error != KM_ERROR_OK) {
        KM1_delete_key(linaro_key.handle, sizeof(linaro_key.handle));
    }

    return error;
}

keymaster_error_t linaro_get_key_characteristics(
        const struct keymaster1_device* dev __unused,
        ...
        keymaster_key_characteristics_t** characteristics)
{
    ...
    error = LinaroParseKeyBlob(blob, additional_params,
...
keymaster_error_t linaro_import_key(const struct keymaster1_device* dev __unused,
        const keymaster_key_param_set_t* params,
        ...
        keymaster_key_characteristics_t** characteristics)
{
    ...
    blob = new KeymasterKeyBlob(reinterpret_cast<uint8_t*>(&linaro_key), sizeof(linaro_keyblob_t));
    if (!blob) {
        error = KM_ERROR_MEMORY_ALLOCATION_FAILED;
        goto out;
    }

    error = LinaroCreateKeyBlob(authorizations, KM_ORIGIN_IMPORTED, *blob,
            &ret_blob, &hw_enforced, &sw_enforced);
    if (error != KM_ERROR_OK) {
        LOG_E("LinaroCreateKeyBlob failed !", 0);
        goto out;
    }
#if 0
...
keymaster_error_t linaro_export_key(const struct keymaster1_device* dev __unused,
        ...
        keymaster_blob_t* export_data)
{
    keymaster_error_t error = KM_ERROR_OK;
    keymaster_algorithm_t algorithm;
    const linaro_keyblob_t* linaro_key = nullptr;
    uint8_t* buf = nullptr;
    uint32_t buf_len = 0;
    ...
    CHK_ERR_AND_LEAVE(error, "LinaroParseKeyBlob failed", out);
#if 0
    keymaster_key_param_set_t tmp1, tmp2;
    hw_enforced.CopyToParamSet(&tmp1);
    sw_enforced.CopyToParamSet(&tmp2);
...
#endif
    linaro_key = reinterpret_cast<const linaro_keyblob_t *>(key_material.begin());
    ...
    error = KM1_export_key(linaro_key->handle, sizeof(linaro_key->handle),
			linaro_key->optee_obj_type, linaro_key->key_len,
            &buf, &buf_len);
...
keymaster_error_t linaro_delete_key(const struct keymaster1_device* dev __unused,
        const keymaster_key_blob_t* key)
{
    ...
    error = KM1_delete_key(linaro_key->handle, sizeof(linaro_key->handle));
...
keymaster_error_t linaro_delete_all_keys(const struct keymaster1_device* dev __unused)
{
    LOG_D("%s:%d:\n", __func__, __LINE__);
    return KM_ERROR_OK;
}
...
static keymaster_error_t linaro_sym_authorized_purpose(
        const AuthorizationSet& sw_enforced,
        const AuthorizationSet& hw_enforced,
        const keymaster_purpose_t purpose)
{
...
static keymaster_error_t get_and_validate_rsa_params(const AuthorizationSet& begin_params,
        ...
        const AuthorizationSet& hw_enforced,
        const keymaster_purpose_t purpose,
        const linaro_keyblob_t* linaro_key,
        keymaster_padding_t* padding,
        keymaster_digest_t* digest)
{
    ...
    switch (*padding) {
        ...
        case KM_PAD_RSA_PSS:
            ...
            key_len = linaro_key->key_len >> 3; /* in bytes */
...
keymaster_error_t linaro_begin(
        const struct keymaster1_device *dev __unused,
        keymaster_purpose_t purpose,
        const keymaster_key_blob_t *key,
        ...
        keymaster_operation_handle_t *operation_handle)
{
    ...
    *operation_handle = 0;
    ...
    error = LinaroParseKeyBlob(blob, additional_params,
            ...
            &hw_enforced, &sw_enforced, true);
    if (error != KM_ERROR_OK) {
        LOG_E("LinaroParseKeyBlob failed", 0);
        goto out;
    }
#if 0
    keymaster_key_param_set_t tmp1, tmp2;
    ...
    handles->key_len = linaro_key->key_len;
    ...
    error = KM1_load_key(&handles->key, linaro_key->handle, sizeof(linaro_key->handle),
            linaro_key->optee_obj_type, linaro_key->key_len);
    if (error != KM_ERROR_OK) {
    ...
    if (algorithm == KM_ALGORITHM_RSA) {
        error = get_and_validate_rsa_params(additional_params, sw_enforced, hw_enforced,
                purpose, linaro_key, &padding, &digest);
        CHK_ERR_AND_LEAVE(error, "get_and_validate_rsa_params failed", out);

        handles->padding = padding;

        error = KM1_allocate_operation(handles->key, linaro_key->key_len,
        ...
        if (digest != KM_DIGEST_NONE) {
            if (purpose == KM_PURPOSE_SIGN || purpose == KM_PURPOSE_VERIFY) {
                error = KM1_digest_init(algorithm, linaro_key->key_len, digest, padding, &(handles->digest));
                if (error != KM_ERROR_OK) {
        ...
        *operation_handle = reinterpret_cast<keymaster_operation_handle_t>(handles);
        ...
        keymaster_digest_t digest;
        error = linaro_sym_authorized_purpose(sw_enforced, hw_enforced, purpose);
        if (error != KM_ERROR_OK) {
            goto out;
        }
        ...
        error = KM1_hmac_init(handles->key, linaro_key->key_len,
...
keymaster_error_t linaro_update(
        const struct keymaster1_device* dev __unused,
        keymaster_operation_handle_t operation_handle,
...
#if 0
        LOG_D("[linaro_update]:%d: output->data(%p)\n", __LINE__, output->data);
        LOG_D("[linaro_update]:%d: output->data_length(%u)\n", __LINE__, output->data_length);
#endif
...
keymaster_error_t linaro_finish(const struct keymaster1_device* dev __unused,
        keymaster_operation_handle_t operation_handle,
        const keymaster_key_param_set_t* in_params __unused,
        ...
        keymaster_key_param_set_t* out_params, keymaster_blob_t* output)
{
    ...
    if (algorithm == KM_ALGORITHM_RSA) {
        ...
        if (handles->block_mode == KM_MODE_GCM) {
            ...
            if (aad_buf.available_read()) {
                /* Aad should be pushed in linaro_update() for most cases.
                   In case of no input in linaro_update(), this is the last chance to push
                   aad out. */
                error = KM1_ae_update_aad(handles->op, aad_buf.peek_read(), aad_buf.available_read());
                CHK_ERR_AND_LEAVE(error, "KM1_ae_update_aad failed", out);
                aad_buf.advance_read(aad_buf.available_read());
...
keymaster_error_t linaro_abort(const struct keymaster1_device* dev __unused,
        keymaster_operation_handle_t operation_handle)
{
    am_operations_t* handles = nullptr;
    keymaster_error_t error = KM_ERROR_OK;
    ...
    if (!operation_handle) {
        LOG_E("linaro_abort: invlid input\n", 0);
