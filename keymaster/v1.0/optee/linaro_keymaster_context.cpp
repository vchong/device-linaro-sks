...
static keymaster_error_t LinaroTranslateAuthorizationSetError(AuthorizationSet::Error err) {
	switch (err) {
...
static keymaster_error_t LinaroSetAuthorizations(const AuthorizationSet& key_description,
    ...
    return LinaroTranslateAuthorizationSetError(sw_enforced->is_valid());
}

keymaster_error_t LinaroCreateKeyBlob(
	...
	error = LinaroSetAuthorizations(key_description, origin, hw_enforced, sw_enforced);
	if (error != KM_ERROR_OK)
		return error;

	AuthorizationSet hidden;
	error = LinaroBuildHiddenAuthorizations(key_description, &hidden);
	if (error != KM_ERROR_OK)
		return error;

	return LinaroSerializeIntegrityAssuredBlob(key_material, hidden, *hw_enforced, *sw_enforced, blob);
}

keymaster_error_t LinaroParseKeyBlob(
	...
	AuthorizationSet hidden;
	keymaster_error_t error = LinaroBuildHiddenAuthorizations(additional_params, &hidden);
	if (error != KM_ERROR_OK)
		return error;

	// Assume it's an integrity-assured blob (new software-only blob, or new keymaster0-backed
	// blob).
	error = LinaroDeserializeIntegrityAssuredBlob(blob, hidden, key_material, hw_enforced, sw_enforced, verify);
	return error;
//	return ParseKeymaster1HwBlob(blob, additional_params, key_material, hw_enforced,
//			sw_enforced);
}

keymaster_error_t LinaroBuildHiddenAuthorizations(
	...
	hidden->push_back(TAG_ROOT_OF_TRUST, reinterpret_cast<const uint8_t*>(root_of_trust_.data()),
			root_of_trust_.size());
#endif
	return LinaroTranslateAuthorizationSetError(hidden->is_valid());
}
